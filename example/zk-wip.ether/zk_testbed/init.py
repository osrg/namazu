...
def make_zoo_cfg(zkid, zkensemble, participant_outside_ensemble=False):
    def server(id, participant):
        return 'server.%d=%s:2888:3888:%s;2181' % (id, zkid2ip(id), 'participant' if participant else 'observer')
    cfg = ZOO_CFG
    for i in zkensemble:
        cfg += server(i, participant=True) + '\n'
    if not zkid in zkensemble:
        cfg += server(zkid, participant=participant_outside_ensemble) + '\n'
    return cfg

def main(zkid, zkensemble, overwrite):
    if (not os.path.isfile('conf/zoo.cfg')) or overwrite:
#        cfg = make_zoo_cfg(zkid, zkensemble, participant_outside_ensemble=True) # this line really differs from zookeeper-2212
        cfg = make_zoo_cfg(zkid, zkensemble, participant_outside_ensemble=False)
